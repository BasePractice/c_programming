...
static UA_UInt32 detector_value = 0;

void write_detector_value(UA_UInt32 value);
...
read_variable(UA_Server *server,
              ...
              UA_DataValue *data) {
    UA_Variant_setScalarCopy(&data->value, &detector_value, &UA_TYPES[UA_TYPES_UINT32]);
    data->hasValue = true;
...
write_variable(UA_Server *server,
    ...
    if (data->value.type == &UA_TYPES[UA_TYPES_UINT32]) {
        detector_value = *(UA_UInt32 *) data->value.data;
...
void register_objects(UA_Server *server) {
    ...
    }
    {
        ...
        type_attributes.dataType = UA_TYPES[UA_TYPES_UINT32].typeId;
